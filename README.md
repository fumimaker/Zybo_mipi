...
- MIPI CSI-2カメラから画像をUDPを通してRaspiberryPI4に送信してRaspiで描画すること。
  - MIPIカメラからデータをVDMAを通してメモリに書く。
  - メモリからPSのアプリでデータを読む。
  - PSのアプリでLWIPを使ってUDPパケットを送信する。
  - RaspiでUDP受信をしてFrameBufferに出力する。

# 比較実験

RapsiでMIPIカメラから送った場合と比較。
...
- [ ] 60FPSで動かしたかった
  - [x] Read割り込みでupdateAddressとかやってみたが、結局30fps以上出なかった。
  - [x] 無理やり常に送信状態にしたら40fpsでたが帯域的にこれが限界そう。
  - [x] 常に送信にすれば速度出てくれるのでやっぱり割り込み的な問題な気がする。
  - [x] Read割り込みにカウンタ置くとちゃんと60になるのに実際にFrameFinishのところでカウンタ置くと30になってしまうの意味が分からない。割り込みの挙動が謎。
- [x] viadoで色を交換するようにする
  - [x] ー＞前書いたコード参考？
- [ ] YUV420にしたりMJPEGを使って帯域を節約したい。
  - [ ] 計算上では12bitなので半分になってくれるはず。
- [ ] 解像度を動的に変更できるようにしたい。
  - [ ] スイッチなどで回路を切り替えて何とか..
  - [ ] クロック回りの変更
  - [ ] VDMAの変更
  - [ ] PSからかなり設定変更しないといけないので、EMIOでつないでやったほうがよさそう。

# やらないといけないこと

- 評価用のRaspi4からMIPIカメラで送信するシステム

  - 一般的な組み込み環境を想定した送信システム
  - MIPIから生でデータってとれるの？（不明）

- 遅延を測定するためのシステム

  - Arduinoとかでやっちゃいたい...
  - FPGAとかで測定したほうが良いのか？（AOMさんの論文がそうだったはず？）
    - あれはディスプレイに照度センサを貼って測定してた。
